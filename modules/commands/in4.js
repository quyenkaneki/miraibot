module.exports.config = {
	name: "in4", // Tên lệnh, được sử dụng trong việc gọi lệnh
	version: "2.0", // phiên bản của module này
	hasPermssion: 0, // Quyền hạn sử dụng, với 0 là toàn bộ thành viên, 1 là quản trị viên trở lên, 2 là admin/owner
	credits: "Nam", // Công nhận module sở hữu là ai // Fix nhẹ
	description: "Xem profile FB của bạn", // Thông tin chi tiết về lệnh
	...
	envConfig: {
    key: "leanhtruong_eG9XXDVS2LuktLYrEnGL",
...
module.exports.run = async function({ api, event }) {
  ...
  const gai = ["Em có thể đi theo anh được không? Bởi vì em luôn được cha mẹ bảo là phải theo giấc mơ của mình.",
    ...
    const trai = ["Em có muốn con mình sau này có ADN của anh không?.",
        ...
        "Cho anh hỏi em một chút đ��ợc không?…. Anh trông em rất là quen….Anh nghĩ là? Mình có biết nhau không?(Chém với gái lạ thì chắc chắn sẽ bảo không rồi)….Thế à. Trông em rất giống người yêu tương lai của anh. ?.",
  ...
  const take = await axios.get(`http://api.leanhtruong.net/api/info?api_key=${key}&id=${uid}`);
  var fl = take.data.follow_user ? `${take.data.follow_user}` : "Không công khai";
  var name = take.data.fullname
  var pf = take.data.url_profile
  var hom = take.data.hometown ? `${take.data.hometown}` : "Không công khai";
  var gender = take.data.gender == 'male' ? "Nam" : take.data.gender == 'female' ? "Nữ" : "Không công khai";
    var birthday = take.data.birthday ? `${take.data.birthday}` : "Không công khai";
    var love = take.data.LeAnhTruong_User_Love ? `${take.data.LeAnhTruong_User_Love}` : "Không công khai";
    var location = take.data.location ? `${take.data.location}` : "Không công khai";
  let callback = function () {
			 api.sendMessage({
				body: `◆━━[ 𝗣𝗥𝗢𝗙𝗜𝗟𝗘 ]━━◆\n\n➣ 𝗧𝗲̂𝗻: ${name}\n➣ 𝗜𝗗: ${uid}\n➣ 𝗚𝗶𝗼̛́𝗶 𝘁𝗶́𝗻𝗵: ${gender}\n➣ 𝗡𝗴𝗮̀𝘆 𝘀𝗶𝗻𝗵: ${birthday}\n➣ 𝗙𝗼𝗹𝗹𝗼𝘄: ${fl}\n➣ 𝗠𝗼̂́𝗶 𝗾𝘂𝗮𝗻 𝗵𝗲̣̂: ${love}\n➣ Đ𝗲̂́𝗻 𝘁𝘂̛̀: ${hom}\n➣ 𝗡𝗼̛𝗶 𝗼̛̉: ${location}\n➣ 𝗟𝗶𝗻𝗸 𝗙𝗕: ${pf}\n\n◆━━[  𝗧𝗛𝗜́𝗡𝗛  ]━━◆\n${gai[Math.floor(Math.random()*gai.length)]}`,
